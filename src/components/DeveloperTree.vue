        ...
        if (child.$el.classList[0] === "v-card") {
          ...
          Object.values(child.$el.children).forEach(arrow => {
            if (arrow.className === "arrow") {
              ...
              const lineC = hypotenus(start.x - end.x, start.y - end.y);
              ...
              arrow.style.transform = `rotate(${correctRotate -
                degIncline}deg)`;
          ...
          Object.values(child.$children).forEach(babyChild => {
            if (babyChild.$el.classList[0] === "baby-child") {
              ...
              if (coordsBabyChildElem.x > halfScreen) {
                ...
                const lineA = startBabyChild.x - endBabyChild.x;
                const lineB = startBabyChild.y - endBabyChild.y;
                const lineC = hypotenus(lineA, lineB);
                const incline = Math.asin(lineB / lineC);
                const degIncline = radToDeg(incline);
                console.log(degIncline);
                babyChildElem.style.transform = `rotate(${degIncline}deg)`;
                babyChildElem.style.position = "absolute";
                babyChildElem.style.left = `${lineA}px`;
                babyChildElem.style.top = `${lineB}px`;
                babyChildElem.style.width = `${lineC}px`;
              } else {
